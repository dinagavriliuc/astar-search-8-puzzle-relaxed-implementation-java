...
public class State {
	...
	Tiles[][] SwapTilesPositions(Tiles zeroTile, Tiles otherTile,
		...
		ArrayIndex tempOtherTile = new ArrayIndex(otherTile.currentPosition.getRow(), otherTile.currentPosition.getColumn());
		ArrayIndex tempZeroTile = new ArrayIndex(zeroTile.currentPosition.getRow(), zeroTile.currentPosition.getColumn());
	...
	public List<State> GetSuccessors() {
		...
		if (zeroPos.column != 2) {
			...
			successors.add(new State(MoveRight(temp), this,
					this.g_score + 1));
		}
		if (zeroPos.column != 0) {
			Tiles [][] temp = new Tiles[3][3];
			temp = CopyTilesArrays(tempTilesArray, temp);
			successors.add(new State(MoveLeft(temp), this,
					this.g_score + 1));
		}
		if (zeroPos.row != 2) {
			Tiles [][] temp = new Tiles[3][3];
			temp = CopyTilesArrays(tempTilesArray, temp);
			successors.add(new State(MoveDown(temp), this,
					this.g_score + 1));
		}
		if (zeroPos.row != 0) {
			Tiles [][] temp = new Tiles[3][3];
			temp = CopyTilesArrays(tempTilesArray, temp);
			successors.add(new State(MoveUp(temp), this,
					this.g_score + 1));
		}
